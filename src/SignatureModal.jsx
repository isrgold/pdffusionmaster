...
const SignatureModal = ({ show, onClose, onSubmit, clickPosition }) => {
    ...
    const handleInsertSavedSignature = (signature) => {
        const newElement = {
            id: Date.now(),
            type: 'signature',
            x: clickPosition.x - signature.width / 2,
            y: clickPosition.y - signature.height / 2,
    ...
    const handleSaveToStorage = () => {
        ...
        const key = `signature_${timestamp}`;
    ...
    useEffect(() => {
        if (show) {
            const allKeys = Object.keys(localStorage).filter(k => k.startsWith('signature_'));
            const signatures = allKeys.map(k => {
                const data = localStorage.getItem(k);
                try {
                    return { key: k, ...JSON.parse(data) };
                } catch {
                    return null;
                }
            }).filter(Boolean);
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className={`px-4 py-2 rounded flex items-center gap-2 ${mode === 'draw'
                        ...
                        <div>
                            ...
                            <div className="flex flex-wrap gap-3">
                                {savedSignatures.map(sig => (
                                    <div key={sig.key} className="border rounded p-1 hover:shadow cursor-pointer" onClick={() => handleInsertSavedSignature(sig)}>
